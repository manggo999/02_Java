...
public class PolyService {
	...
	// 자식타입 참조 변수 = 자식 객체 (부모(Car)필드 + 자식(Tesla)필드)	
		
		// Spark 객체 생성
		Spark spark = new Spark();
    // 자식타입 참조 변수 = 자식 객체 (부모(Car)필드 + 자식(Spark)필드)
		
		
		//*****다형성*******
		
		// 부모타입 참조변수 = 자식객체
		Car car2 = new Tesla(); // 오류 발생안함
		...
		// Spark 부모, 본인의 메서드 모두 사용 가능
		
		
		// 1-3) 
		//car2 (부모 = 자식(Tesla)) 
		// Car car2 = new Tesla(); (== Tesla객체가 Car객체로 변화)
		car2.setEngine("전기모터");
		car2.setFuel("전기");
		car2.setWheel(4);
		...
		// car3(부모 = 자식(Spark))
		// Car car3 = new Spark();
		car3.setEngine("경차 엔지");
		car3.setFuel("휘발유");
		car3.setWheel(4);
		// car2.set.discountOffer(0.5);
		// 부모타입 참조변수이기 때문에 Tesla의 고유필드, 메서드 사용 불가능
		...
		// 객체 배열 : 같은 객체 참조 자료형에 변수를 하나의 묶음으로 다루는 것
		// 다형성을 이용한 객체배열 : 
		...
		// 상속 + 다형성
		// 상속 특징 : Car 상속 클래스는 모두 grtEngine() 을 가지고 있다
