...
// ClsService 와 Student는 패키지가 다름
public class ClsService /*extends Student*/ {
	          // 부모로 Student 클래스를 상속받음
	          // Student(부모) -> ClsService(자식, 후손클래스가 되버림)
	...
	public void ex1() {
		...
		// -> 전체에서 직접 접근 가능한 public v1만 에러발생 X
		...
		System.out.println(std.v1); // public - 프로그램 전체에서 접근가능
		
		//System.out.println(std.v2); // Student 클래스를 상속받았으므로 Student의 protected 변수인
		                            // v2는 직접 접근 가능
		
		//System.out.println(std.v3); // (default) 다른패키지 ->에러
		 
		//System.out.println(std.v4); // 해당 클래스가 아닌 다른 클래스 ->에러
	}
	...
	public void ex2() {
		...
		// schoolName에 노란줄이 뜨는 이유(경고) => 제대로 작성 안했기 때문
